...
crawler_name = args["CRAWLER_NAME"]               # <-- CAMBIA si quieres otro nombre
crawler_db = args["CRAWLER_DB"]                   # <-- EJ: trade_data_imat3a05
crawler_role_arn = args["CRAWLER_ROLE_ARN"]       # <-- IMPORTANTE: rol del crawler
table_prefix = (args.get("TABLE_PREFIX") or "").strip()
wait = (args.get("WAIT") or "true").lower() == "true"

# <-- CAMBIA: aquí van los 3 sources (bronze/silver/gold)
...
def start_crawler_if_ready():
    ...
    state = c["State"]  # READY | RUNNING | STOPPING
    if state != "READY":
