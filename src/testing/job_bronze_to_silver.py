...
partition_cols = [c.strip() for c in (args.get("PARTITION_COLS") or settings.PARTITION_COLS).split(",") if c.strip()]
asset_default = (args.get("ASSET_DEFAULT") or settings.DEFAULT_ASSET).strip()
...
# Normalize datetime column (rename to settings.TIME_COL)
time_col_candidates = [settings.TIME_COL, "time", "timestamp", "date", "datetime", "Date"]
time_col = next((c for c in time_col_candidates if c in df.columns), None)
if time_col:
    df = df.withColumn(settings.TIME_COL, F.to_timestamp(F.col(time_col)))
    if settings.TIME_COL != time_col:
        df = df.drop(time_col)
else:
    raise Exception(f"No se encontró columna temporal en BRONZE. Busca alguna de {time_col_candidates}.")

# Normalize close price column (rename to settings.CLOSE_COL)
close_col_candidates = [settings.CLOSE_COL, "Close", "close"]
close_col = next((c for c in close_col_candidates if c in df.columns), None)
if close_col:
    df = df.withColumn(settings.CLOSE_COL, F.col(close_col).cast("double"))
    if settings.CLOSE_COL != close_col:
        df = df.drop(close_col)
else:
    raise Exception(f"No se encontró columna de cierre en BRONZE. Busca alguna de {close_col_candidates}.")

if settings.ASSET_COL not in df.columns:
    if "asset" in df.columns:
        df = df.withColumnRenamed("asset", settings.ASSET_COL)
    elif "Asset" in df.columns:
        df = df.withColumn(settings.ASSET_COL, F.col("Asset"))
    else:
        # Fallback to a fixed value if not present anywhere
        df = df.withColumn(settings.ASSET_COL, F.lit(asset_default))
